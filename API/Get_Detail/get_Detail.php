...
if ($_SERVER["REQUEST_METHOD"] === 'POST') {
    if ($p == 'reservation') {
        ...
        $htmlData = '<div class="h-full w-full mr-2">' .            
            ...
            '<th scope="col" class="px-6 py-3">MIR No</th>' .
            '<th scope="col" class="px-6 py-3">Spool No</th>' .
            '<th scope="col" class="px-6 py-3">MIR Quantity</th>' .
            '</tr>' .
            '</thead>' .
            '<tbody>';

        $total = 0;
        for ($i = 0; $i < count($data['Sorted']); $i++) {
            $total += $data['Sorted'][$i]['MIR Qty'];
            $htmlData .= '<tr class="bg-white border-b">' .
                '<td class="px-6 py-4">' . ($i + 1) . '</td>' .
                '<td class="px-6 py-4">' . $data['Sorted'][$i]['MIR No'] . '</td>' .
                '<td class="px-6 py-4">' . $data['Sorted'][$i]['Spool No'] . '</td>' .
                '<td class="px-6 py-4">' . $data['Sorted'][$i]['MIR Qty'] . '</td>' .
                '</tr>';
        }
